...
export default function SocialPage() {
  ...
  const tabs = [
    { id: 'friends' as const, label: 'Friends', icon: Users, count: mockFriends.length, color: 'from-blue-500 to-indigo-600' },
    { id: 'discover' as const, label: 'Discover', icon: UserPlus, count: filteredSuggestions.length, color: 'from-green-500 to-emerald-600' },
    { id: 'activity' as const, label: 'Activity', icon: MessageCircle, count: mockActivity.length, color: 'from-purple-500 to-violet-600' },
              ...
              >
                ...
                {count > 0 && id !== 'stats' && (
                  <span className={`text-xs rounded-full px-2 py-0.5 ml-1 ${
                    activeTab === id ? 'bg-white/20' : 'bg-white text-gray-800'
                  }`}>
                    {count}
                  </span>
                )}
        ...
        {activeTab === 'friends' && (
          <div className="space-y-6">
            <div className="text-center mb-6">
              <h2 className="text-3xl font-bold text-white mb-2">Your Friends ({mockFriends.length})</h2>
              <p className="text-white/80 text-lg">Connect and battle with your poetry companions</p>
            </div>
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
              {mockFriends.map((friend, index) => (
                <div 
                  key={friend.id} 
                  className="bg-white/95 backdrop-blur-sm rounded-2xl shadow-xl p-6 transform hover:scale-105 transition-all duration-200 animate-slideInUp border border-white/50"
                ...
                >
                  <div className="flex items-center justify-between mb-4">
                    <div className="flex items-center space-x-3">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-gray-600 font-medium">{friend.points} points</p>
                      </div>
                    </div>
                  ...
                  <Button className="w-full bg-gradient-to-r from-blue-500 to-indigo-600 hover:from-blue-600 hover:to-indigo-700" size="sm">
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
              {filteredSuggestions.map((suggestion, index) => (
                ...
                >
                  <div className="flex items-center justify-between mb-4">
                    <div className="flex items-center space-x-3">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-gray-600 font-medium">{suggestion.points} points</p>
                      </div>
                    </div>
                    <div className="text-right">
                      <div className="text-sm font-bold text-emerald-600">{suggestion.similarity}% match</div>
                  ...
                  <div className="mb-4">
                    ...
                    <div className="flex flex-wrap gap-1 mb-2">
                      {suggestion.commonThemes.map((theme, i) => (
                        <span key={i} className="px-2 py-1 bg-gradient-to-r from-indigo-100 to-purple-100 text-indigo-800 rounded-full text-xs font-medium">
                          {theme}
                        </span>
                      ))}
                    </div>
                  ...
                  <div className="flex space-x-2">
                    <Button className="flex-1 bg-gradient-to-r from-emerald-500 to-teal-600 hover:from-emerald-600 hover:to-teal-700" size="sm">
                      Add Friend
                    </Button>
          ...
          <div className="bg-white/95 backdrop-blur-sm rounded-2xl shadow-2xl overflow-hidden border border-white/50">
            ...
            <div className="divide-y divide-gray-100">
              {mockActivity.map((activity, index) => (
                <div 
                  key={activity.id} 
                  className="px-8 py-6 hover:bg-gray-50 transition-all duration-200 transform hover:scale-[1.01] animate-slideInLeft"
                  style={{ animationDelay: `${index * 0.05}s` }}
                >
                  <div className="flex items-center space-x-4">
                    <div className="text-3xl transform hover:scale-110 transition-all duration-200">
                      {activity.icon}
                    </div>
                    ...
                    <div className={`px-3 py-1 rounded-full text-xs font-bold ${
                      activity.type === 'like' ? 'bg-gradient-to-r from-red-100 to-pink-100 text-red-800' :
                      activity.type === 'friend' ? 'bg-gradient-to-r from-blue-100 to-indigo-100 text-blue-800' :
          ...
          <div className="space-y-8">
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
              <div className="bg-gradient-to-br from-blue-500 to-indigo-600 rounded-2xl shadow-xl p-6 text-center transform hover:scale-105 transition-all duration-200 text-white">
                ...
                <p className="text-4xl font-bold">{mockFriends.length}</p>
            ...
            <div className="bg-white/95 backdrop-blur-sm rounded-2xl shadow-xl p-8 border border-white/50">
              ...
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  <span className="text-gray-600 font-medium">Friend Requests Sent</span>
                  <span className="font-bold text-gray-900">12</span>
                </div>
                <div className="flex items-center justify-between">
                  <span className="text-gray-600 font-medium">Friend Requests Received</span>
                ...
                <div className="flex items-center justify-between">
                  <span className="text-gray-600 font-medium">Collaboration Invites</span>
